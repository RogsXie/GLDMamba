...
# ===================================================================
# Patch 数据集
# 修正：__getitem__ 直接返回差分 patch，而非两个独立 patch
# ===================================================================
class ChangePatchDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        # 修正：在数据集端计算差分 patch，与论文 Fig.2 输入处理阶段一致
        diff = np.abs(p1 - p2)
        diff = np.transpose(diff, (2, 0, 1)).astype(np.float32)
        return torch.from_numpy(diff), torch.tensor(y, dtype=torch.long)


# ===================================================================
# 验证
# 修正：解包单个 x（差分 patch），去掉 x2
# ===================================================================
@torch.no_grad()
def evaluate_val(model, loader, criterion):
    model.eval()
    total_loss, correct, total = 0.0, 0, 0
    for x, y in loader:                          # 修正：x1,x2 → x
        x, y        = x.to(device), y.to(device)
        logits      = model(x)                   # 修正：model(x1,x2) → model(x)
...
def infer_full_map(model, data1, data2, gt, ws=9, infer_batch=2048):
    ...
    while i < len(coords):
        ...
        b    = len(bc)
        diff = np.zeros((b, C, ws, ws), dtype=np.float32)   # 修正：只保留 diff
        for j, (r, c) in enumerate(bc):
            rp, cp = r + pad, c + pad
            p1 = np.transpose(d1p[rp-pad:rp+pad+1, cp-pad:cp+pad+1, :], (2,0,1))
            p2 = np.transpose(d2p[rp-pad:rp+pad+1, cp-pad:cp+pad+1, :], (2,0,1))
            diff[j] = np.abs(p1 - p2)                        # 修正：计算差分
        logits = model(torch.from_numpy(diff).to(device))    # 修正：只传 diff
...
def run_one_seed(seed, run_idx, data1_std, data2_std, gt, H, W, C, class_count):
    ...
    net = modelpa430.GASSM_PatchNet(in_channels=C, num_classes=2).to(device)
    ...
    for epoch in range(1, epochs + 1):
        ...
        for x, y in train_loader:  # 修正：x1,x2,y → x,y
            x, y = x.to(device), y.to(device)
            optimizer.zero_grad()
