...
class deeplab_V2(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(in_channels=242, out_channels=64, kernel_size=3, padding=1),
...
if __name__ == "__main__":
    ...
    # 1) 实例化你的模型（注意：你的类名是 Finalmodel，不是 Net）
    model = Finalmodel().to(device)
    model.eval()

    # 2) 构造一对双时相输入：t0, t1
    #    shape: (B, C, H, W)
    t0 = torch.randn(B, C, H, W, device=device)
    t1 = torch.randn(B, C, H, W, device=device)

    print(f"输入形状: t0={t0.shape}, t1={t1.shape}")
    ...
    # 5) 计算 MACs / FLOPs / Params（可选）
    if profile is not None:
        # thop 要求输入的 batch 维度也要在
        macs, params = profile(model, inputs=(t0, t1), verbose=False)

        print(f"计算量 MACs：{macs/1e6:.4f} M")
        print(f"计算量 FLOPs：{2*macs/1e6:.4f} M")   # 1 MAC ≈ 2 FLOPs（常用近似）
