...
class SS2D1(nn.Module):
    ...
    def forward_corev0(self, x: torch.Tensor):
        ...
        # xs = xs.float().view(B, -1, L).contiguous()
        # dts = dts.float().view(B, -1, L).contiguous()
        dts = dts.float().reshape(B, -1, L).contiguous()
        # Bs = Bs.float().view(B, K, -1, L).contiguous()
        # Cs = Cs.float().view(B, K, -1, L).contiguous()
        xs = xs.float().reshape(B, -1, L).contiguous()
        Bs = Bs.float().reshape(B, K, -1, L).contiguous()
        Cs = Cs.float().reshape(B, K, -1, L).contiguous()
        Ds = self.Ds.float().view(-1).contiguous()
        As = -torch.exp(self.A_logs.float()).view(-1, self.d_state).contiguous()
        dt_projs_bias = self.dt_projs_bias.float().view(-1).contiguous()
    ...
    def forward_corev1(self, x: torch.Tensor):
        ...
        xs = xs.float().view(B, -1, L).contiguous()  # <-- contiguous added
        dts = dts.float().view(B, -1, L).contiguous()  # <-- contiguous added
        Bs = Bs.float().view(B, K, -1, L).contiguous()  # <-- contiguous added
        Cs = Cs.float().view(B, K, -1, L).contiguous()  # <-- contiguous added
        Ds = self.Ds.float().view(-1).contiguous()  # <-- contiguous added
        As = -torch.exp(self.A_logs.float()).view(-1, self.d_state).contiguous()  # <-- contiguous added
        dt_projs_bias = self.dt_projs_bias.float().view(-1).contiguous()  # <-- contiguous added
