...
# # 实例化
# block = VSSBlock(dim=64).cuda()
#
# # 输入 [B, C, H, W]
# x = torch.randn(64, 64, 11, 11).cuda()
#
# # 前向
# out = block(x)
#
# print(f"输入: {x.shape}")    # [64, 32, 11, 11]
# print(f"输出: {out.shape}")  # [64, 32, 11, 11]
...
class VSSM(nn.Module):
    def __init__(
        ...
        self.conv3d = nn.Sequential(
            nn.Conv3d(64, 96, (3, 3, 3), padding=(1, 1, 1)),
            nn.BatchNorm3d(96),
            nn.GELU(),
        )
    ...
    @staticmethod
    def _make_patch_embed(in_chans=3, embed_dim=96, patch_size=3, patch_norm=True, norm_layer=nn.LayerNorm,
                          channel_first=False):
    ...
    @staticmethod
    def _make_patch_embed_v2(in_chans=3, embed_dim=96, patch_size=3, patch_norm=True, norm_layer=nn.LayerNorm,
                             channel_first=False):
        # if channel first, then Norm and Output are both channel_first
        # stride = patch_size // 2
        stride=3
